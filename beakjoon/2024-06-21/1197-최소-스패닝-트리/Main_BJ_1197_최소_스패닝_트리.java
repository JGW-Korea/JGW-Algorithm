...
public class Main_BJ_1197_최소_스패닝_트리 {
  ...
  // Union-Find 알고리즘
  public static int find(int x) { // 최상위 정점 찾는(Find) 알고리즘 (경로 최적화를 동시에 진행)
    if(parent[x] == x) {
      return x;
    }
  ...
  public static void union(int a, int b) { // 병합(Union) 알고리즘
    a = find(a);
    b = find(b);
  ...
  public static boolean compare(int a, int b) { // 같은 집합인지 판별(Compare)하는 알고리즘
    a = find(a);
    b = find(b);
